...
node("s1d1_kitchen", 1,
     ...
     [ch("d1k_straight", "Tell her where you actually are with it.",
         nxt="s1d1_kitchen_react", set_state={"mumTrust": 1, "discipline": 1, "answered_mum": True},
         scores=(10, 0, 15)),
      ch("d1k_manage", "Give her the version that ends the conversation.",
         nxt="s1d1_kitchen_react", set_state={"mumStrain": 1, "answered_mum": True},
         scores=(5, 5, -5)),
      ch("d1k_lie", "Tell her it's handled.",
         nxt="s1d1_kitchen_react", set_state={"mumStrain": 2, "lied_to_mum": True, "discipline": -1},
         scores=(0, 5, -15))])

node("s1d1_kitchen_react", 1,
...
node("s1d2_ask", 2,
     ...
     [ch("d2a_help", "Come. Don't ask.", nxt="s1d2_help",
         set_state={"jayTrust": 1, "heat": 2, "helped_jay": True, "credit": 1}, scores=(0, 10, -5)),
      ch("d2a_limited", "Come, but you're not holding anything.", nxt="s1d2_limited",
         set_state={"jayTrust": 1, "heat": 1, "helped_jay": True}, scores=(15, 5, 10)),
      ch("d2a_decline", "No.", nxt="s1d2_decline",
         set_state={"jayStrain": 2, "jayTrust": -1}, scores=(5, -10, 10)),
      ch("d2a_redirect", "Not that. I'll do the other thing instead.", nxt="s1d2_redirect",
...
node("s1d2_kai", 2,
     ...
     [ch("d2k_answer", "Answer it.", nxt="s1d2_close",
         set_state={"kaiStrain": 2, "rep": 1}, scores=(0, 5, 0)),
      ch("d2k_past", "Walk past.", nxt="s1d2_close",
         set_state={"kaiStrain": 1, "walked_away": True, "rep": -1}, scores=(10, 0, 5)),
      ch("d2k_defuse", "Say the thing that ends it.", nxt="s1d2_close",
         set_state={"rep": 1, "walked_away": True, "credit": 1}, scores=(15, 10, 10))])

node("s1d2_close", 2, P + "Home. Nothing has changed that anyone could point to.",
     cont("s1d3_open"))
...
node("s1d3_open", 3, P + "Sunday. Word comes that you should come by.",
     cont("s1d3_arrive"), interstitial={"label": "Sunday", "durationMs": 2000})
...
node("s1d3_dee", 3,
     ...
     [ch("d3d_impress", "Give him the version that sounds like something.",
         nxt="s1d3_impress", set_state={"standing": "known", "deeTrust": 2, "rep": 1}, scores=(5, 10, -5)),
      ch("d3d_true", "Tell him the actual answer.",
         nxt="s1d3_true", set_state={"standing": "around", "deeTrust": 1}, scores=(15, 0, 15)),
      ch("d3d_leave", "Say very little and leave early.",
         nxt="s1d3_leave", set_state={"standing": "new", "deeStrain": 1, "rep": -1}, scores=(5, 5, 5))])

node("s1d3_impress", 3, P + "He likes it. You can feel exactly how much, and how easy it would be to keep doing.",
     cont("s1d3_react"))
node("s1d3_true", 3, P + "He doesn't flinch. He says something about his own eighteenth year that you don't know how to hold.",
     cont("s1d3_react"))
node("s1d3_leave", 3, P + "He walks you to the door and is nice about it, which is worse.",
     cont("s1d3_react"))
...
node("s1d3_tell", 3,
     ...
     [ch("d3t_tell", "Tell him.", nxt="s1d3_close",
         set_state={"jayTrust": 1, "told_jay_about_dee": True}, scores=(10, -5, 15)),
      ch("d3t_partial", "Tell him a version.", nxt="s1d3_close",
         set_state={"jayStrain": 1}, scores=(5, 10, -5)),
      ch("d3t_nothing", "Nothing.", nxt="s1d3_close",
         set_state={"jayStrain": 2, "jayTrust": -1}, scores=(0, 5, -10))])

node("s1d3_close", 3, P + "Sunday ends the way Sundays do.", cont("s1d4_open"))
...
node("s1d4_open", 4,
     ...
     cont("s1d4_choose"), interstitial={"label": "Monday", "durationMs": 2000},
...
node("s1d4_choose", 4,
     ...
     [ch("d4c_amara", "Amara.", nxt="s1d4_amara_1",
      ...
      ch("d4c_tunde", "Tunde's session.", nxt="s1d4_tunde_1",
         set_state={"dayFourWent": "tunde", "discipline": 2, "amaraStrain": 1}, scores=(10, 0, 10)),
      ch("d4c_kai", "The thing with Kai.", nxt="s1d4_kai_1",
         set_state={"dayFourWent": "kai", "kaiStrain": 1, "heat": 1, "discipline": -1}, scores=(0, 10, -5)),
      ch("d4c_none", "None of it.", nxt="s1d4_none_1",
         set_state={"dayFourWent": "nowhere", "amaraStrain": 1, "tundeStrain": 1, "discipline": -1,
                    "rep": -1, "tundeTrust": -1, "missed_training": True},
         scores=(0, -5, 0))])

# --- lane A: Amara
node("s1d4_amara_1", 4, P + "The bus stop, then somewhere neither of you picked. She asks a question about Dee that she should not know to ask.",
     [ch("d4a_honest", "Answer it.", nxt="s1d4_amara_2", set_state={"amaraTrust": 2}, scores=(10, -5, 15)),
      ch("d4a_deflect", "Move it along.", nxt="s1d4_amara_2", set_state={"amaraStrain": 2}, scores=(5, 10, -10))])
node("s1d4_amara_2", 4, P + "Later, walking back. She says the thing she came to say.",
     cont("s1d4_after"),
     variants=[{"when": {"amaraBand": "tight"}, "message": P + "She says it plainly, and waits."}])

# --- lane B: Tunde
node("s1d4_tunde_1", 4, P + "The cage. Tunde has brought someone who runs the programme he keeps talking about.",
     [ch("d4t_ask", "Ask about it properly.", nxt="s1d4_tunde_2",
         set_state={"tundeTrust": 2, "discipline": 1, "credit": 1}, scores=(15, 5, 10)),
      ch("d4t_hang", "Stay at the edge of it.", nxt="s1d4_tunde_2",
         set_state={"tundeStrain": 1}, scores=(0, 0, 0))])
node("s1d4_tunde_2", 4, P + "Afterwards Tunde is careful not to ask whether you meant it.",
     cont("s1d4_after"),
...
# --- lane D: nowhere
node("s1d4_none_1", 4, P + "Your room. Three conversations you are not in.",
     [ch("d4n_watch", "Follow it on your phone anyway.", nxt="s1d4_none_2", set_state={"heat": 1}, scores=(0, 5, -5)),
      ch("d4n_off", "Put it face down.", nxt="s1d4_none_2", set_state={"discipline": 1, "rep": -1}, scores=(5, -5, 5))],
     presentation={"type": "messages", "modal": True,
                   "data": {"location": "Your room", "character": {"name": "", "role": "", "mood": "flat"}}})
node("s1d4_none_2", 4, P + "By eleven it has happened without you.", cont("s1d4_after"))

node("s1d4_after", 4,
     P + "Tuesday morning. Two people ask you where you were.",
     [ch("d4s_true", "Tell them.", nxt="s1d4_close", set_state={"dayFourSaid": "true"}, scores=(10, -5, 15)),
      ch("d4s_lie", "Don't.", nxt="s1d4_close",
         set_state={"dayFourSaid": "lied", "amaraStrain": 1, "tundeStrain": 1, "tundeTrust": -1}, scores=(0, 10, -15)),
      ch("d4s_silent", "Say nothing at all.", nxt="s1d4_close", set_state={"dayFourSaid": "silent"}, scores=(5, 5, 0))],
...
node("s1d4_close", 4, P + "Tuesday closes quietly.", cont("s1d5_open"))
...
node("s1d5_open", 5,
     ...
     cont("s1d5_stairwell"), interstitial={"label": "Wednesday", "durationMs": 2000},
...
node("s1d5_limited_n", 5, P + "He agrees to the condition immediately, which tells you what the condition was worth.", cont("s1d5_react"))
node("s1d5_declined", 5, P + "He is not angry. He is disappointed in a way that costs more than anger would.", cont("s1d5_react"))
node("s1d5_redirect_n", 5, P + "You spend what you were owed. He takes it, and something in the room resets.", cont("s1d5_react"))
...
node("s1d6_open", 6,
     ...
     cont("s1d6_news"), interstitial={"label": "Thursday", "durationMs": 2000},
...
node("s1d6_act", 6,
     ...
     [ch("d6a_front", "Front it.", nxt="s1d6_fronted",
         set_state={"daySixAct": "fronted", "heat": 2, "jayTrust": 2, "rep": 2, "credit": 1},
         scores=(5, -10, 20)),
      ch("d6a_cover", "Keep it off you and off him.", nxt="s1d6_covered",
         set_state={"daySixAct": "covered", "heat": 1, "jayTrust": 1, "lied_to_mum": True, "rep": 1},
         scores=(15, 10, -10)),
      ch("d6a_sit", "Let it sit.", nxt="s1d6_sat",
         set_state={"daySixAct": "sat", "jayStrain": 2, "jayTrust": -1, "rep": -1},
         scores=(0, 5, -15))])

node("s1d6_fronted", 6, P + "You say it was you. It is true enough to survive being said out loud.", cont("s1d6_tell_first"))
node("s1d6_covered", 6, P + "You put a version together that holds. It holds.", cont("s1d6_tell_first"))
node("s1d6_sat", 6, P + "You do nothing, and doing nothing turns out to be a thing you did.", cont("s1d6_tell_first"))

node("s1d6_tell_first", 6,
     P + "Someone has to hear it from you first.",
     [ch("d6t_mum", "Mum.", nxt="s1d6_close", set_state={"mumTrust": 2, "answered_mum": True}, scores=(10, -5, 15)),
      ch("d6t_jay", "Jay.", nxt="s1d6_close", set_state={"jayTrust": 1}, scores=(5, 5, 10)),
      ch("d6t_tunde", "Tunde.", nxt="s1d6_close", set_state={"tundeTrust": 1, "tundeStrain": 1}, scores=(5, 0, 5)),
      ch("d6t_none", "Nobody.", nxt="s1d6_close", set_state={"mumStrain": 1, "jayStrain": 1, "tundeStrain": 2}, scores=(0, 10, -10))])
...
node("s1d7_open", 7,
     P + "Friday. A week.",
     cont("s1d7_situation"), interstitial={"label": "Friday", "durationMs": 2400},
...
node("s1d7_move", 7,
     ...
     [ch("d7m_pay", "Pay it and be done.",
         ...
         nxt="s1d7_paid", set_state={"money": -40, "paid_it_back": True, "heat": -2}, scores=(10, 10, 10)),
      ch("d7m_call", "Call in what you're owed.",
         ...
         nxt="s1d7_called", set_state={"credit": -2, "rep": 1}, scores=(15, 10, 5)),
      ch("d7m_jay", "Go to Jay.",
         requires=[{"jayBand": "tight"}, {"jayBand": "steady"}],
         locked="Go to Jay. — you two aren't there any more",
         nxt="s1d7_jay", set_state={"jayTrust": 1, "heat": 1}, scores=(5, 5, 10)),
      ch("d7m_alone", "Handle it yourself.", nxt="s1d7_alone",
         set_state={"heat": 1, "rep": 1}, scores=(5, 0, 5))])

node("s1d7_paid", 7, P + "It costs what it costs and then it is closed.", cont("s1d7_final"))
node("s1d7_called", 7, P + "Two calls. Both answered. You notice that you counted.", cont("s1d7_final"))
node("s1d7_jay", 7, P + "Jay does not ask what it is for.", cont("s1d7_final"),
     variants=[{"when": {"told_jay_about_dee": True}, "message": P + "Jay does not ask what it is for, because you told him in March."}])
node("s1d7_alone", 7, P + "You do it on your own, which works, and costs something you can't name yet.", cont("s1d7_final"))
