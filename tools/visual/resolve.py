...
def resolve_scene_token(node, mapping):
...
def resolve_props(node, mapping):
    authored = node.get("presentation", {}).get("data", {}).get("visual", {})
    out, unknown = [], []
...
def resolve(node_id, node, registries, mapping, variant_index=None):
    ...
    scene_tok = resolve_scene_token(node, mapping)
    scene_def = registries["scenes"].get(scene_tok)
    if scene_def is None:
        raise ResolveError(f"{node_id}: unknown scene token {scene_tok}")
    render["scene"] = {"id": scene_tok, "time": scene_def["default_time"]}

    props, unknown = resolve_props(node, mapping)
    render["props"] = props
    if unknown:
