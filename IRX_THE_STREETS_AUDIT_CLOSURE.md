...
**Mandatory prefix convention: `s<scenario>d<day>_<descriptor>`.**

The Streets is a set of **independent seven-day scenarios**, each with its own Day 1. A
scenario-blind `d<day>_` prefix therefore collides by construction the moment Scenario 2
exists — and `renderBlocks.blockFor` resolves `node[key] || node.default || null`, so the
collision does not throw. It silently returns the other scenario's artwork. The scenario
number is what prevents that; A11 stays as the backstop and is **not** relaxed.

| Scenario | Prefix | Status |
|---|---|---|
| `the_secret` | *(none)* | grandfathered, unchanged |
| `the_instruction` | *(none)* | grandfathered, unchanged |
| `the_streets` (Scenario 1) | `s1d<n>_` | e.g. `s1d5_the_ask`, `s1d5_reaction` |
| The Streets, Scenario 2 | `s2d<n>_` | its own Day 1 through Day 7 |
| The Streets, Scenario 3 | `s3d<n>_` | likewise |
...
## Framing contract — intro modal and establishing node

**Status: recorded as a Phase 3 / Day 1 deliverable. Not written. Do not write it against the
skeleton.**

Scenario 1 must carry the same two-layer framing the three shipped scenarios use. It currently has
neither: `s1d1_open` is a 118-character placeholder, and `scenario_intros.dart` has no
`the_streets` key at all.

**The missing intro fails silently, which is why it is recorded here.** `ScenarioIntros.forScenario`
returns `null` for an unknown id and the caller skips the modal rather than showing an empty one —
deliberate, so the dev picker can load anything on disk. Nothing errors, nothing warns, and the
validator does not see the Flutter repo. Without a tracked deliverable, Scenario 1 ships dropping
the player straight into Day 1 with no framing at all: exactly the problem the modal was built to
solve after playtesting.

### The two layers do different jobs and must not duplicate each other

| | Answers | Lives in |
|---|---|---|
| **Intro modal** | *What am I about to play, and how does this experience work?* | `irx-flutter/lib/content/scenario_intros.dart`, key `the_streets` |
| **Establishing node** | *What is happening to me, who is involved, and why am I here?* | `s1d1_open` in `the_streets.json` |

**Layer 1 — the intro modal.** Fields: `eyebrow`, `title`, `situation`, `youAre`, `duration`,
`rules`. Authored client-side, per the model's own note: `getRootView` strips `premise`,
`estimatedMinutes` and `cast`, and intro copy is a different register from a spec-doc premise line.
`ScenarioIntro.fromJson` exists so moving this server-side later is a change of data source rather
than a rewrite.

**Layer 2 — the establishing node.** `s1d1_open` must do for Scenario 1 what `establish_northstar`
does for The Instruction and `establish` does for The Secret: plant the relationships and the
pressure that the week then acts on. The Instruction spends 463 characters across five paragraphs
establishing a debt to Ray that predates the scenario by six weeks. The current placeholder
establishes nothing and must not survive into production.
...
**The standing rule from `scenario_intros.dart` binds here too: situation, never skill.** The Prince
must not mention verification; The Instruction must not mention documentation; The Secret must not
mention loyalty. The skill under test in The Streets is **accumulation** — that small decisions
compound. So the intro may describe the seven-day *structure* and must never state that choices add
up, which is the thing being measured.

### Authoring order — narrative premise first

The intro must not be written against the current skeleton, because the actual situation has not
been authored yet. Sequence:

1. Determine the real narrative premise of Scenario 1.
2. Then write the intro copy, the establishing node and the rest of Day 1 **as one coherent
   opening**, checked against each other for duplication.

### Two values deliberately left open

- **`estimatedMinutes: 35` is not final player-facing wording.** `duration` is a free display
  string. The Instruction reads "~10 minutes" and means one sitting; 35 minutes across seven
  sessions is a different shape and should be described as such. Decide with the copy.
...
The Streets is a collection of **independent seven-day scenarios**. Scenario 1 is the current
`the_streets` story. Scenarios 2 and 3 are future independent stories with their own Day 1 through
Day 7, their own cast and their own outcomes, and are not part of this phase. Persistence occurs
**within a scenario run only** — no state, relationship or consequence carries between scenarios.

---

## Final authoring contract

1. Every node declares `day` (1–7) and a node ID prefixed `s<scenario>d<day>_`.
...
| 6 | **Day 1 as one coherent opening** — narrative premise → intro copy (`scenario_intros.dart`, key `the_streets`) → establishing `s1d1_open` → Day 1 prose → first environment + first figure | No duplication between the two framing layers; no hidden state, band or trajectory named in the intro; `duration` and the seven-day `rules` entry finalised; occlusion alpha test; 390pt readability; asset locked |
